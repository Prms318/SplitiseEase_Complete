  ...
  return <div className="auth-shell">
    ...
    <main className="auth-main">
      <div className="auth-form-wrap">
        ...
        <h2>{mode === 'login' ? 'Welcome back.' : mode === 'invitation' ? 'You’re invited.' : 'Make it official.'}</h2>
        <p className="auth-intro">{mode === 'login' ? 'Sign in to see what your circles are up to.' : mode === 'invitation' ? 'Choose a password to accept your SplitEase invitation.' : 'Create an account to start sharing expenses.'}</p>
        <form className="auth-form" onSubmit={submit}>
          ...
          {mode !== 'invitation' && <label>EMAIL ADDRESS<input autoComplete="email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label>}
          ...
          <button className="button button-primary auth-submit" type="submit" disabled={busy}>{busy ? 'Connecting...' : mode === 'login' ? 'Sign in' : mode === 'invitation' ? 'Accept invitation' : 'Create account'}{!busy && <ArrowRight size={16} />}</button>
        </form>
        {mode !== 'invitation' && <div className="auth-switch"><span>{mode === 'login' ? 'New to SplitEase?' : 'Already have an account?'}</span><button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError('') }}>{mode === 'login' ? 'Create account' : 'Sign in'} <UserRoundPlus size={14} /></button></div>}
        {mode === 'login' && <div className="auth-demo-note"><span className="pulse-dot" /><span>Local demo account is prefilled. Run <code>services/tests/seed-demo.ps1</code> once to load sample groups and expenses.</span></div>}
      </div>
    </main>
  </div>
}